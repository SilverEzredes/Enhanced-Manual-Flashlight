...
local modUpdated = "01/30/2026"
local modVersion = "v1.0.6"
...
local function draw_Flashlight_GUI()
    if imgui.tree_node("Flashlight Settings") then
        ...
        imgui.text_colored(modVersion .. " | " .. modUpdated, 0xFFFFFFFF); imgui.same_line(); imgui.text("(c) " .. modAuthor .. " ")
        imgui.spacing()
        imgui.indent(-5)
        imgui.end_rect(3)
        imgui.tree_pop()
...
local function draw_OnFrame_Flashlight_GUI()
    if (flashlight_settings.isBatteryGUI and flashlight_settings.flashlight_BatteryLevel < flashlight_settings.batteryGUIThreshold) or flashlight_settings.isAlwaysBatteryGUI then
        ...
        if should_DrawBatteryGUI or not flashlight_settings.isDynamicBatteryGUI then
            ...
            imgui.push_style_color(7, convert_rgba_to_ABGR(flashlight_settings.batteryGUIBackgroundColor))
            imgui.push_style_color(43, convert_rgba_to_ABGR(flashlight_settings.batteryGUIForegroundColor))
